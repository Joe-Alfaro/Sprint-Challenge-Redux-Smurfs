...
Actions are the actual http requests that send data to the reducers. The reducers then return the intended data to the store. The store is where data is kept for global access.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

Application state is state that all components will have access to. I think that if data is only prevelant to one component it should only be stored there for security reasons. You don't want to give too much information everywhere.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

Thunk is a middleware that lets action creators return functions. That allows https requests to be dispatched asynchronously.
